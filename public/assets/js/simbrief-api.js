...
function do_simbriefsubmit(outputpage) {
    ...
    if (api_code == null || api_code == false || typeof (api_code) == 'undefined') {
        api_code = 'notset';
        sb_res_load('/ops/simbrief/api-req?api_req=' + document.getElementsByName('orig')[0].value + document.getElementsByName('dest')[0].value + document.getElementsByName('type')[0].value + timestamp + outputpage_calc);
        setTimeout(function () {
            do_simbriefsubmit(outputpage);
        }, 500);
        return;
...
function Redirect_caller() {
    ...
    */
    ...
    if (fe_result == null || fe_result == false || typeof (fe_result) == 'undefined') {
        fe_result = 'notset';
        sb_res_load('/ops/simbrief/js-url-check?js_url_check=' + ofp_id + "&var=fe_result");
        setTimeout(function () {
            Redirect_caller();
        }, 500);
        return;
